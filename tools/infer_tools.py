...
class DiffusionSVC:
    ...
    def load_model(self, f0_model=None, f0_min=None, f0_max=None):
        ...
        # else:
        self.vocoder, self.args = load_model_vocoder("configs/", device=self.device)
    ...
    def infer_from_audio(self, audio, sr=44100, key=0, spk_id=1, spk_mix_dict=None, aug_shift=0,
                         infer_speedup=10, method='dpm-solver', k_step=None, use_tqdm=True,
                         spk_emb=None, threhold=-60, index_ratio=0,naive_state=None, wavenet_state=None,gaussian=None):
        ...
        if k_step is not None:
            ...
            gt_spec = None
        @partial(jax.jit)
        def naive_infer(units,f0):
            rng = jax.random.PRNGKey(1234)
            params_key,r_key,dropout_key,rng = jax.random.split(rng,4)
            init_rngs = {'params': params_key, 'dropout': dropout_key,'rnorms':r_key}
            out_audio = naive_state.apply_fn({'params': naive_state.params},ppg=units,f0=f0,infer=True,mutable=False,rngs=init_rngs)
            return out_audio
    ...
    def infer_from_long_audio(self, audio, sr=44100, key=0, spk_id=1, spk_mix_dict=None, aug_shift=0,
                              infer_speedup=10, method='dpm-solver', k_step=None, use_tqdm=True,
                              spk_emb=None,
                              threhold=-60, threhold_for_split=-40, min_len=5000, index_ratio=0,naive_state=None, wavenet_state=None,gaussian=None):
        ...
        for segment in tqdm(segments):
            ...
            @partial(jax.jit)
            def naive_infer(units,f0,vol):
                rng = jax.random.PRNGKey(1234)
                params_key,r_key,dropout_key,rng = jax.random.split(rng,4)
                init_rngs = {'params': params_key, 'dropout': dropout_key,'rnorms':r_key}
                naive_mel = naive_state.apply_fn({'params': naive_state.params},ppg=units,f0=f0,volume=vol,infer=True,mutable=False,rngs=init_rngs)
                return naive_mel
            seg_units =jnp.asarray(seg_units.cpu().numpy())
            print(seg_units.shape)
            seg_f0 = jnp.asarray(seg_f0.cpu().numpy())
            seg_volume = jnp.asarray(seg_volume.cpu().numpy())
            mel_naive = naive_infer(seg_units,seg_f0,seg_volume)
            mel_naive = mel_naive.transpose(0,2,1)
            mel_diff = gaussian.pred_sample(predict_key, wavenet_state,mel_naive,mel_naive.shape)
            mel_diff = mel_diff.squeeze(0)
            #mel_diff = mel_diff.squeeze(0)
            mel_diff = mel_diff.transpose(0,2,1)
            print(mel_diff)
            print(mel_diff.shape)
            out_mel = torch.from_numpy(np.asarray(mel_diff)).to(self.device)
            seg_f0 = torch.from_numpy(np.asarray(seg_f0)).to(self.device)
            seg_output = self.mel2wav(out_mel, seg_f0)
            _left = start_frame * self.args.data.block_size
            _right = (start_frame + seg_units.shape[1]) * self.args.data.block_size
